...
```{r}
mussels <- read.csv("L:/Noah Johnson/stats/u/mussels.csv")
...
```{r}
young_air <- air %>% as_tibble() %>% filter(age == 1) %>% select(-age)

youngModel <- glm(n.error ~ n.operator, family = poisson, data = young_air)
young.coef <- summary(youngModel)$coefficients
print(young.coef)

print(sprintf("The expected number of errors for an air traffic control staff who works at a site with 100 operators and who is younger than 55 is %.2f.", exp(young.coef[[1,1]] + young.coef[[2,1]] * 100)))
```

```{r}
old_air <- air %>% as_tibble() %>% filter(age == 2) %>% select(-age)

oldModel <- glm(n.error ~ n.operator, family = poisson, data = old_air)
old.coef <- summary(oldModel)$coefficients
print(old.coef)

print(sprintf("The expected number of errors for an air traffic control staff who works at a site with 100 operators and who is older than 55 is %.2f.", exp(old.coef[[1,1]] + old.coef[[2,1]] * 100)))
```
...
This statistic should follow a $\chi^2$ distribution with 1 degree of freedom (the difference in number of parameters between the two models). So what are the chances that we see a value this large just by chance?
